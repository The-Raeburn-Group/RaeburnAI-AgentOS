...
function requireDatabaseUrl(): string {
  ...
  if (!databaseUrl.startsWith('postgresql://') && !databaseUrl.startsWith('postgres://')) {
    throw new Error('AgentOS disaster recovery supports PostgreSQL DATABASE_URL values only.');
  }
  return databaseUrl;
}
...
function parseCli(argv: string[]): CliOptions {
  ...
  if (!['seed', 'backup', 'restore', 'verify'].includes(command ?? '')) {
    throw new Error('Usage: postgres-dr.ts <seed|backup|restore|verify> [options]');
  }
...
async function readManifest(path: string): Promise<BackupManifest> {
  const parsed = JSON.parse(await readFile(path, 'utf8')) as Partial<BackupManifest>;
  if (
...
async function createBackup(dumpPath: string, manifestPath: string): Promise<void> {
...
async function restoreBackup(
  ...
  if (!allowDestructiveRestore) {
    throw new Error('Restore is destructive; pass --allow-destructive-restore explicitly.');
  }
  const manifest = await readManifest(manifestPath);
  if ((await sha256File(dumpPath)) !== manifest.dumpSha256) {
    throw new Error('Backup dump hash does not match its manifest; restore aborted.');
  }
...
async function verifyRestore(dumpPath: string, manifestPath: string): Promise<void> {
  const manifest = await readManifest(manifestPath);
  if ((await sha256File(dumpPath)) !== manifest.dumpSha256) {
  ...
  if (JSON.stringify(restored) !== JSON.stringify(manifest.snapshot)) {
    throw new Error('Restored AgentOS database does not match the source backup manifest.');
  }
}
...
async function seedRecoveryFixture(): Promise<void> {
  ...
  try {
    await prisma.tenant.deleteMany({ where: { id: { in: [...FIXTURE_TENANTS] } } });

    await prisma.tenant.createMany({
      data: [
        { id: tenantA, name: 'DR Tenant A', slug: 'dr-tenant-a', createdAt, updatedAt: createdAt },
        { id: tenantB, name: 'DR Tenant B', slug: 'dr-tenant-b', createdAt, updatedAt: createdAt },
      ],
    });
...
async function main(): Promise<void> {
  ...
  if (options.command === 'restore') {
    await restoreBackup(options.dumpPath, options.manifestPath, options.allowDestructiveRestore);
    return;
  }
