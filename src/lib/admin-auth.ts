...
export const humanRoles = ["admin", "operator", "approver", "auditor", "viewer"] as const;
...
const rolePermissions: Record<HumanRole, readonly HumanPermission[]> = {
  ...
  operator: ["agent.read", "workflow.read", "workflow.run", "approval.read", "metrics.read"],
  approver: ["agent.read", "workflow.read", "approval.read", "approval.decide", "audit.read"],
  auditor: ["agent.read", "workflow.read", "approval.read", "audit.read", "metrics.read"],
...
export function normalizeRoles(value: unknown): HumanRole[] {
  ...
  const roles = values.filter(
    (role): role is HumanRole => typeof role === "string" && humanRoles.includes(role as HumanRole),
  );
...
export function hasPermission(identity: HumanIdentity, permission: HumanPermission): boolean {
  return identity.roles.some((role) => rolePermissions[role].includes(permission));
}

export async function requireHumanPermission(permission: HumanPermission): Promise<HumanIdentity> {
  ...
  if (!hasPermission(identity, permission)) throw new HumanAuthError("forbidden");
  return identity;
}

const issuer = configured(process.env.AGENTOS_OIDC_ISSUER) ?? "https://oidc.invalid";
const clientId = configured(process.env.AGENTOS_OIDC_CLIENT_ID) ?? "unconfigured";
const clientSecret = configured(process.env.AGENTOS_OIDC_CLIENT_SECRET) ?? "unconfigured";
const rolesClaim = configured(process.env.AGENTOS_OIDC_ROLES_CLAIM) ?? "roles";
const tenantClaim = configured(process.env.AGENTOS_OIDC_TENANT_CLAIM) ?? "tenant_id";

export const humanAuthOptions: NextAuthOptions = {
  secret: configured(process.env.NEXTAUTH_SECRET) ?? "unconfigured-development-secret",
