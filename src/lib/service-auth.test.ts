...
describe('requireChainServiceToken', () => {
  it('fails closed when the service credential is not configured', async () => {
    ...
    const response = requireChainServiceToken(new Request('http://localhost/api/workflows/run'));
  ...
  it('rejects a missing or incorrect bearer credential', () => {
    ...
    const missing = requireChainServiceToken(new Request('http://localhost/api/workflows/run'));
